...
#### FUNCTIONS ####
function usage {
	printf "usage: dp.sh -p <project-name> -d <commit-sha> [--not-simulate]\n"
	printf "\t -p, --project  \tThe name of the project to deploy (look at the configuration file)\n"
	printf "\t -d, --deploy   \tThe commit you want to deploy \n"
	printf "\t --not-simulate \tReally deploy, it will not simulate!\n"
    printf "\t --no-colors    \tDisable colors layout\n"
}
...
#then
#	rm -Ir $REPO_CLONED_DIR
#	echo  $REPO_CLONED_DIR
#fi
#ls -l  $REPO_CLONE_DIR	
...
echo ==c_deploy===$C_DEPLOY=====
echo ===filename==$FILENAME_DIFF====
echo ====" git diff --name-status $C_PRODUCTION $C_DEPLOY > $EXPORTED_DIR/$FILENAME_DIFF " ======
